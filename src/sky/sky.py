...
class Sky:
    ...
    def getStudentEnrollments(
        ...
        if not isinstance(students, list):
            raise Exception('Value of students must either be a user id or list of user ids')
        ...
        for user_id in students:
            ...
            if isinstance(student_enrollment, pd.DataFrame):
                ...
                enrollment = pd.concat([enrollment, student_enrollment], ignore_index=True)
        return enrollment
    ...
    def enrollmentMedley(
        ...
        academic_enrollments = enrollments.loc[enrollments.offering_type_id.isin(offerings.loc[offerings.description == 'Academics', 'id'].tolist())]
        advisory_enrollments = enrollments.loc[enrollments.offering_type_id.isin(offerings.loc[offerings.description == 'Advisory', 'id'].tolist())]

        advisory_enrollments = advisory_enrollments.loc[advisory_enrollments.changed_sections == 0]

        # Cleaning the department names
        departments = academic_enrollments.departments.explode().apply(
            pd.Series).drop(
            [0 , 'id'], 
            axis=1).rename(columns={'name':'department_name'})
        ...
        academic_enrollments = academic_enrollments.loc[
            academic_enrollments.duration_name == term,
            [
                'user_id', 'block_name', 'course_title', 'faculty_first_name', 
                'faculty_last_name', 'duration_name', 'id', 'department_name',
                'section_identifier'
            ]].rename(columns={
                'id':'section_id'
            }).dropna(
        ...
        advisory_enrollments = advisory_enrollments.loc[
            ...
            [
                'user_id', 'course_title', 'faculty_first_name', 
                'faculty_last_name', 'id'
            ]].rename(columns={
                'id':'section_id'
            }).dropna(
                axis=0,
        ...
        athletics_enrollments = enrollments.loc[
            enrollments.offering_type_id.isin(offerings.loc[offerings.description == 'Athletic', 'id'].tolist()),
            ['user_id', 'course_title', 'faculty_first_name', 
            'faculty_last_name', 'id', 'duration_name'
            ]].rename(columns={
                    'id':'section_id'
                }).dropna(
                    axis=0,
    ...
    def getOfferingId(
        self,
        offeringType: Union[str, list] = "Academics"
        ) -> list:
